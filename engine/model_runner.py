...
class ModelRunner:
    ...
    #             [1, 2, 3, 7, 8, 9],
    #             [0, 2, 3, 7, 8, 9]]
    # seq0 is the first seq in this batch, thus BlockManager.cache is empty.
    # seq0 cache miss, seqlen_q = 6, cu_seqlens_k = 6. And cache these tokens.
    # seq1 cache hit[1, 2, 3], thus [1, 2, 3] needn't cache => seqlen_q = 3 => cache[4, 5, 6]
    # but 
    ...
    def prepare_decode(self, seqs: list[Sequence]):
        ...
        for seq in seqs:
            ...
            # append token's physical kvcache block address generated by pre-step
            slot_mapping.append(seq.block_table[-1] * self.block_size + seq.last_block_num_tokens - 1)
        
        input_ids = torch.tensor(input_ids, dtype=torch.int32, pin_memory=True).cuda(non_blocking=True)
        positions = torch.tensor(positions, dtype=torch.int32, pin_memory=True).cuda(non_blocking=True)
    ...
    def run_model(self, input_ids: torch.Tensor, position: torch.Tensor, is_prefill: bool):
    ...
    def run(self, seqs: list[Sequence], is_prefill: bool)->list[int]:
        input_ids, position = self.prepare_prefill(seqs) if is_prefill else self.prepare_decode(seqs)
        temperatures = self.prepare_sample(seqs) if self.rank == 0 else None
        logits = self.run_model(input_ids, position, is_prefill)
        token_ids = self.sampler(logits, temperatures).tolist() if self.rank == 0 else None
        reset_context()
        return token_ids

    # only can use by decode
