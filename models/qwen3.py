...
from layers.linear import QKVParallelLinear, RowParallelLinear, MergedColumnParallelLinear
...
class Qwen3Attention(nn.Module):
    def __init__(
        ...
        # just calculate each gpu's partial weight
        # the weight shape is [(num_heads + 2*num_kv_heads) * head_dim, hidden_size]
        # return [N, (num_heads + 2*num_kv_heads) * head_dim], N is tokens' number
...
class Qwen3ForCausalLM(nn.Module):
    ...
    # return hidden_state, whose shape is [batch_size(seqs.len), hidden_size]
    def forward(self, input_ids: torch.Tensor, positions) -> torch.Tensor:
        return self.model(input_ids, positions)
    
    # Compute similarity between predicted value and vocab, and sample
    def compute_logits(self, hidden_state: torch.Tensor):
